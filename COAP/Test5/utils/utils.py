...
# ---------- compare_ssim ------------
def compare_SSIM(org, est, on_y=False, gray_scale=False):
    assert (on_y==False or gray_scale==False), "Is your image RGB or gray? please choose and try again"
    if on_y:
        return compare_ssim_y(np_to_pil(org), np_to_pil(est))
    if gray_scale:
        return compare_ssim(np.mean(org, axis=0), np.mean(est, axis=0))
    return compare_ssim(org, est)
...
def compare_ssim_y(org_pil, est_pil):
    return compare_ssim(rgb2ycbcr(org_pil), rgb2ycbcr(est_pil))
